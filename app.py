...
def load_models():
    base_path = os.path.dirname(__file__)
    scaler_path = os.path.join(base_path, "models", "rfm_scaler.pkl")
...
if input_method == "Nhập mã khách hàng":
    ...
    if customer_id in df_rfm.index:
        try:
            ...
            st.success(
                f":bar_chart: Cụm khách hàng số: **{cluster_label}** – {cluster_name}"
            )
...
# --- Cách 2: Nhập slider ---
    ...
    try:
        ...
        for i, row in df_customer.iterrows():
            st.success(
                f" Khách hàng {i+1} thuộc cụm: **{row['Cluster']} – {row['Phân nhóm']}**"
            )
...
# --- Cách 3: Tải file CSV ---
    ...
    st.markdown(
        " **Yêu cầu:** File cần có 3 cột: `Recency`, `Frequency`, `Monetary`. Cột `CustomerID` là tùy chọn."
    )
    uploaded_file = st.file_uploader(
        "Tải lên file CSV chứa thông tin RFM", type=["csv"]
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            if not required_cols.issubset(df_uploaded.columns):
                ...
                df_uploaded["Phân nhóm"] = df_uploaded["Cluster"].apply(
                    interpret_cluster
                )
                ...
                for i, row in df_uploaded.iterrows():
                    st.info(
                        f" Khách hàng {row.get('CustomerID', i+1)} thuộc cụm: **{row['Cluster']} – {row['Phân nhóm']}**"
                    )
